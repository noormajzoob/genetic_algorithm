...
class IsolateWorkerPool<T, R> {
  ...
  Future<List<R>> sendMany(List<IsolateTask<T, R>> tasks) async {
    ...
    return Future.wait(futures.map((e) => e!));
  }
}
...
abstract class IsolateTask<T, R> {
  ...
  IsolateTask(this.payload) : id = IsolateTask._random.nextInt(maxInt);

  Future<R> task(T payload);

  Future<R> execute() async => result ?? await task(payload);
}
